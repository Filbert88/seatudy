...
const StudentBar:React.FC<StudentBarProps> = ({students = []}) => {
    ...
    const dummyImage = "https://res.cloudinary.com/dl2cqncwz/image/upload/v1723449492/pmswrzvs7sh2u5wzgnjx.jpg";
                            ...
                            students.map((student) => (
                                <li key = {student.id} >
                                    <div className = "flex items-center p-2">
                                        <img src = {dummyImage} alt = "profile" className = "w-10 h-10 rounded-full bg-white"/>
                                        <div className = "flex flex-col ml-4">
                                            <div className = "font-normal">{autoFormatName(student.user.fullName)}</div>
                                            {student.progress.length > 0 ? (<div className = "text-xs font-light">{student.progress[student.progress.length - 1].progressPct}</div>):(<div className = "text-xs font-light">0%</div>)}
                                        </div>
                                    </div>
                                </li>
                            ))
