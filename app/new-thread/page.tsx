...
const ViewForumPage = () => {
  ...
  const postCourse = async (materialTitle: string, pdfUrl: string, courseId: string) => {
    try {
      setIsLoading(true);
      ...
      <div className="min-h-screen w-screen flex flex-row bg-primary text-secondary font-nunito">
        ...
        {!isLoading && (
          <div className="flex flex-col h-screen pl-[18rem] pt-[6rem] w-full pr-20 pb-10 scroll overflow-hidden">
            ...
            <input type="text" onChange={(e) => { setTitle(e.target.value) }} value={title} className="w-full h-10 rounded-lg px-3 mb-10" placeholder="" name="title" required />
            <div className="text-xl font-semibold mb-3">
              {'Content'}
            </div>
            ...
            <div>
              <button onClick={async () => await handleSubmit()} className="bg-fourth text-white font-semibold px-10 py-2 rounded-lg mt-10">Submit</button>
            </div>
          </div>
        )}
      </div>
