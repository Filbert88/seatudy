...
import { CourseInterface, ForumCommentInterface, ForumPostInterface, SideBarDataInterface } from "@/components/types/types";
...
const ViewForumPage = () => {
  ...
  const getCommentData = async (postId: string) => {
    try {
      ...
      if (!response.ok) {
        alert("Failed to fetch forum data");
      }
      const data = await response.json();
      setCommentData(data);
    } catch (error) {
                ...
                <div key={index} className="flex flex-col bg-white shadow-lg rounded-md my-3 p-3 justify-between">
                  ...
                  <>
                    ...
                    <div className="my-2 ml-2 bg-primary p-2 pt-1 rounded-md w-[40%]">
                      ...
                      <textarea value={commentFieldValue} onChange={(e) => {setCommentFieldValue(e.target.value)}} className="mt-1 py-1 px-2 rounded-md h-10 w-full"/>
                      ...
                      className="mt-2 bg-fourth text-white text-sm rounded-md py-1 px-3">Reply</button>}
                    </div>
                    <div className="mt-1 ml-2">
                      {commentData.map((comment: any, index: number) => {
                        return (
                          <div key={index} className="flex flex-col border-t-2 p-3 justify-between">
                            <div className="text-sm font-semibold mb-1">{new Date(comment.createdAt).toLocaleString()} - {comment.user.fullName}</div>
