    ...
    <div className="w-screen min-h-screen bg-[url('/assets/login_bg.png')] bg-cover py-20 px-[15rem] flex flex-col font-nunito">
      ...
      <div className="rounded-3xl flex bg-third flex-grow h-full">
        ...
        <div className="w-full rounded-r-3xl p-10">
          ...
          <input type="email" onChange={handleChange} value={email} className="w-full h-10 rounded-lg px-3 mb-10" placeholder="Enter your email address.." name="email" required/>

          <div className="font-semibold text-white text-xl mb-3">Password</div>
          <input type="password" onChange={handleChange} value={password} className="w-full h-10 rounded-lg px-3 mb-8" placeholder="Enter your password.." name="password" required/>

          <div className="flex justify-between">
            <a className="mr-2 text-primary">{"Don't have an account yet?"}</a>
            <a className="underline text-primary hover:cursor-pointer mr-auto" onClick={handleRegisterClick}>Sign Up here</a>
            <div className="underline text-primary hover:cursor-pointer">Forgot Password</div>
          </div>
          
          <div onClick={handleLoginClick} className="bg-fourth mt-8 w-fit px-10 py-2 text-white font-bold rounded-lg hover:cursor-pointer">Sign In</div>
        </div>
      </div>
    </div>
  );
